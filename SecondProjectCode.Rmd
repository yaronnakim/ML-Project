...
FirstTree <- rpart(y~.,data=train.data, method = 'class',parms = list(prior = proportions))
...
```{r}
Practice.Tree <- rpart(y~.,dataset)
(confusion <- table(Predicted = predict(Practice.Tree, type = 'class'), true = dataset$y))
missclasification(confusion)
