...
Creating the new variables from Variables that are currently extremely odd shaped and unhelpful
```{r}
...
```{r}
ggplot(cleandata, aes(y, colour = y)) +  geom_bar()  + ylab('Proportion') + facet_grid(vars(HasFee)) 

cleandata %>% names

mosaicplot(table(cleandata$y,cleandata$Vaccinated), color = T)
